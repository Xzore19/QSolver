...
def check_assertion(qc):
    ...
    assert 0.6+delta > temp_dir.get(0, 0) > 0.6-delta
    assert 0.1+delta > temp_dir.get(1, 0) > 0.1-delta
    assert 0.2+delta > temp_dir.get(2, 0) > 0.2-delta
    assert 0.1+delta > temp_dir.get(3, 0) > 0.1-delta
    print("Assertion SAT!!!!!!!")
